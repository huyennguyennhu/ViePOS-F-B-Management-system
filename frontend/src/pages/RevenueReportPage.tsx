  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="pie-tooltip">
        ...
        <p className="pie-tooltip-value">{new Intl.NumberFormat('vi-VN').format(data.revenue)}đ</p>
...
const PaymentPieTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="pie-tooltip">
        <p className="pie-tooltip-title">{data.name}</p>
        <p className="pie-tooltip-label">{formatPaymentPieLine(data.orders, data.amount)}</p>
      </div>
    );
  }
  return null;
...
export default function RevenueReportPage() {
  ...
  const chartData = (statsData?.chartData || statsData?.hourlyData || statsData?.dailyData || []).map((d: any) => ({
    time: d.time,
  ...
  const chartTitle = chartGranularity === 'day'
    ? 'Tổng Quan Doanh Thu (theo ngày)'
    : 'Tổng Quan Doanh Thu (theo giờ)';
  ...
  const paymentData = statsData ? withPaymentPieColors(buildPaymentPieSlices(statsData, orders)) : [];
  const hasPaymentPieData = paymentData.length > 0;
  const hasBarData = chartData.some((d: any) => d.revenue > 0);
  ...
  const formatCurrencyFull = (v: number) => new Intl.NumberFormat('vi-VN').format(v) + 'đ';
    ...
    <div className="revenue-report-container">
      <div className="sticky-header">
        ...
        <div className="revenue-filter-card">
          ...
          <div className="date-input-wrapper">
            ...
            <div className="date-input-inner">
              <input type="date" value={fromDate} max={toDate}
                onChange={(e) => { setFromDate(e.target.value); setFilterType('Tùy chỉnh'); }}
          ...
          <div className="date-input-wrapper">
            ...
            <div className="date-input-inner">
              <input type="date" value={toDate} max={todayStr}
                onChange={(e) => { setToDate(e.target.value); setFilterType('Tùy chỉnh'); }}
                className="date-input" />
            </div>
          </div>
      ...
      <div className="revenue-metrics-grid">
        <div className="metric-card">
          ...
          <div className="metric-value">{isLoading ? '---' : formatCurrencyFull(totalRevenue)}</div>
          <div className="metric-badge-container">
            <span className="metric-comparison">Từ {orders.filter(o=>o.status==='COMPLETED').length} đơn hoàn tất</span>
          </div>
        </div>
        ...
        <div className="metric-card">
          ...
          <div className="metric-value red-text">{isLoading ? '---' : `${totalOrders} đơn`}</div>
          <div className="metric-badge-container">
        ...
        <div className="metric-card">
          <div className="metric-header">
            <div className="metric-icon-bg green-bg">
              <img src={iconAverage} alt="Trung Bình" className="metric-icon-img" />
            </div>
            <span className="metric-title">Trung Bình Giá Trị Đơn</span>
          </div>
          <div className="metric-value green-text">{isLoading ? '---' : formatCurrencyFull(avgOrderValue)}</div>
          <div className="metric-badge-container">
        ...
        <div className="metric-card">
          <div className="metric-header">
            <div className="metric-icon-bg green-bg">
              <img src={iconProduct} alt="Tổng Đơn Kỳ" className="metric-icon-img" />
            </div>
            <span className="metric-title">Tổng Đơn (Kỳ)</span>
          </div>
          <div className="metric-value green-text">{isLoading ? '---' : orders.length}</div>
          <div className="metric-badge-container">
      ...
      <div className="dashboard-section chart-section">
        ...
        <div className="chart-wrapper">
          {isLoading ? (
            ...
            <div className="chart-empty-state">Chưa có doanh thu trong khoảng thời gian đã chọn</div>
          ) : (
        ...
        <div className="dashboard-section pie-section">
          ...
          <div className="pie-wrapper">
            {isLoading ? (
              ...
              <div className="chart-empty-state">Chưa có đơn hoàn tất trong khoảng thời gian đã chọn</div>
            ) : (
              <ResponsiveContainer width="99%" height={250}>
                <PieChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  <Pie data={typeData} cx="50%" cy="50%" innerRadius={40} outerRadius={75}
                    paddingAngle={2} dataKey="value" stroke="none" labelLine={false} label={renderPieLabel}>
        ...
        <div className="dashboard-section pie-section">
          ...
          <div className="pie-wrapper">
            {isLoading ? (
              ...
              <div className="chart-empty-state">Chưa có dữ liệu thanh toán trong khoảng thời gian đã chọn</div>
            ) : (
              <ResponsiveContainer width="99%" height={250}>
                <PieChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  <Pie data={paymentData} cx="50%" cy="50%" innerRadius={40} outerRadius={75}
                    paddingAngle={2} dataKey="value" stroke="none" labelLine={false} label={renderPaymentPieLabel}>
                    {paymentData.map((entry: any, index: number) => (
                      <Cell key={`pay-cell-${index}`} fill={paymentPieColorForLabel(entry.name)} />
                    ))}
                  </Pie>
      ...
      <div className="dashboard-section top-products-section">
        ...
        <div className="table-responsive">
          <table className="dashboard-table">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '20px', color: '#888' }}>Đang tải...</td></tr>
                ...
                <tr key={i}>
                  ...
                  <td>{p.qty} {p.unit || 'ly'}</td>
                </tr>
              )) : (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '20px', color: '#888' }}>Chưa có dữ liệu doanh thu sản phẩm</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="dashboard-section order-list-section">
        ...
        <div className="table-responsive">
          <table className="dashboard-table full-table">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '20px', color: '#888' }}>Đang tải...</td></tr>
                ...
                const typeLabel = SESSION_TYPE_LABEL[sessionTypeKey] || sessionTypeKey;
                ...
                const typeStyle = typeColors[typeLabel] || { bg: '#f0f0f0', color: '#555' };

                const paymentLabel: Record<string, string> = { CASH: 'Tiền mặt', BANK_TRANSFER: 'Chuyển khoản' };
                ...
                return (
                  <tr key={i}>
                    ...
                    <td>
                      <span className="type-badge" style={{ backgroundColor: typeStyle.bg, color: typeStyle.color }}>
                        {typeLabel}
                      </span>
                    </td>
                    <td>{o.paymentMethod ? paymentLabel[o.paymentMethod] || o.paymentMethod : '--'}</td>
                    <td>{formatCurrencyFull(o.totalAmount)}</td>
                    <td>
                      <span className="status-badge" style={{
                        backgroundColor: isCompleted ? '#e8f5e9' : '#fce4e4',
                ...
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '20px', color: '#888' }}>Chưa có đơn hàng trong khoảng thời gian này</td></tr>
              )}
            </tbody>
          </table>
        </div>
